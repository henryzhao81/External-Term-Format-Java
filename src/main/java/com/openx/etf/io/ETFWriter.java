...
import com.openx.etf.erlang.ErlangObject;
...
public class ETFWriter {
  ...
  private void writeToBuffer(byte... data) {
      ...
      if (this.data[0] != version && includeHeader) {
...
//          writeSmallUTF8Atom(atom);
  ...
  public <T> ETFWriter writeTuple(Collection<T> tuple) {
      //if (tuple.size() > 256) //TODO: double check
      if (tuple.size() < 256)
  ...
  public <T> ETFWriter writeTuple(T[] tuple) {
      //if (tuple.length > 256) //TODO: double check
      if (tuple.length < 256)
          writeSmallTuple(tuple);
      else
          writeLargeTuple(tuple);
  ...
  /*TODO : no need reflection for now
  ...
  public ETFWriter write(Object o) {
      if (o == null) {
          ...
          //writeMap(o);//TODO: no need refection for now
          return this;
      }
  ...
  public byte[] toBytes(boolean zip) {
    if (!this.includeHeader && zip) {
      ...
      compresser.end();
      byte[] dest = new byte[compressedDataLength + 6];
      dest[0] = this.version;
      dest[1] = (byte)HEADER;//80
      dest[2] = (byte)(orgLen >> 24);
      dest[3] = (byte)(orgLen >> 16);
      dest[4] = (byte)(orgLen >> 8);
      dest[5] = (byte)(orgLen >> 0);
      System.arraycopy(output, 0, dest, 6, compressedDataLength);
      return dest;
    }
